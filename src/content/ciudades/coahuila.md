---
nombre: "Coahuila"
abreviatura: "COAH"
capital: "Saltillo"
region: "Norte"
estado: "Coahuila"
descripcion: "Empresas de seguridad privada en Coahuila: la autorización estatal dura 1 año y es prorrogable por periodos iguales. Qué exigir antes de contratar."
orden: 9
normativa:
  ley: "Ley del Sistema de Seguridad Pública del Estado, Título Sexto"
  autoridad: "Dirección General de Registro y Control de los Servicios de Seguridad Privada"
  dependencia: "Secretaría de Seguridad Pública del Estado"
  vigencia: "1 año, prorrogable por periodos iguales"
  modalidades: 10
  modalidadesArticulo: "Art. 252"
  registro: "Registro Estatal de Empresas de Seguridad Privada"
  sinLeyEspecifica: true
fuentes:
  - titulo: "Ley del Sistema de Seguridad Pública del Estado de Coahuila"
    url: "https://www.congresocoahuila.gob.mx/transparencia/03/Leyes_Coahuila/coa168.pdf"
---

Consultar un proveedor de seguridad privada en **Coahuila** exige revisar el marco local: la autorización se otorga por entidad y por modalidad, y las reglas cambian de un estado a otro.

## Quién autoriza aquí

Coahuila no cuenta con una ley específica de seguridad privada. La materia se regula mediante **Ley del Sistema de Seguridad Pública del Estado, Título Sexto**, en conjunto con la legislación estatal de seguridad pública.

La instancia que tramita y otorga la autorización es la **Dirección General de Registro y Control de los Servicios de Seguridad Privada**, adscrita a la Secretaría de Seguridad Pública del Estado.

### Datos clave

- **Vigencia de la autorización:** 1 año, prorrogable por periodos iguales.
- **Modalidades reconocidas:** 10 (Art. 252). La autorización ampara solo las modalidades que declara, así que debe coincidir con el servicio que vas a contratar.
- **Registro estatal:** Registro Estatal de Empresas de Seguridad Privada.

> No hay ley específica: la materia vive en el Título Sexto de la ley de seguridad pública, con última reforma en junio de 2025. El trámite se gestiona en la plataforma SIRESP.

## Si tu proveedor opera en más de un estado

La autorización estatal solo ampara el territorio de Coahuila. Cuando una empresa presta servicios en dos o más entidades federativas aplica además la Ley Federal de Seguridad Privada, cuya autorización es un documento distinto con vigencia de un año. Pide el que corresponda al lugar donde vas a recibir el servicio.
